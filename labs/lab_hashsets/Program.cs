...
namespace Lab8_Sets
{
    ...
    public class SetOperationsLab
    {
        ...
        public int DeduplicateEmails(List<string> emailList)
        {
            ...
            if (emailList == null) return 0;

            var seen = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            var unique = new List<string>();

            foreach (var raw in emailList)
            {
            ...
            int removed = emailList.Count - unique.Count;

            // Modify the provided list in-place to show the deduplicated result
            emailList.Clear();
            emailList.AddRange(unique);
        ...
        public bool HasPermission(string userId, string permission)
        {
            ...
            if (string.IsNullOrWhiteSpace(userId) || string.IsNullOrWhiteSpace(permission)) return false;

            if (!userPermissions.ContainsKey(userId)) return false;

            var perms = userPermissions[userId];
            return perms != null && perms.Contains(permission.Trim());
        }
        ...
        public int AddPermissions(string userId, HashSet<string> newPermissions)
        {
            ...
            if (string.IsNullOrWhiteSpace(userId) || newPermissions == null || newPermissions.Count == 0) return 0;

            // Ensure we operate with a case-insensitive set for permissions
            if (!userPermissions.ContainsKey(userId))
            {
                ...
                foreach (var p in newPermissions)
                {
                    ...
                    if (set.Add(p.Trim())) added++;
                }
                userPermissions[userId] = set;
                return added;
            }

            // Existing user - merge permissions
            var existing = new HashSet<string>(userPermissions[userId], StringComparer.OrdinalIgnoreCase);
            ...
            userPermissions[userId] = existing;
        ...
        public HashSet<string> GetMissingPermissions(string userId, HashSet<string> requiredPermissions)
        {
            ...
            if (requiredPermissions == null || requiredPermissions.Count == 0) return missing;

            // Start with required permissions (normalized)
            foreach (var p in requiredPermissions)
            {
                if (!string.IsNullOrWhiteSpace(p)) missing.Add(p.Trim());
            }

            if (!userPermissions.ContainsKey(userId))
            {
                // User doesn't exist - all required permissions are missing
                return missing;
            }

            var userPerms = new HashSet<string>(userPermissions[userId], StringComparer.OrdinalIgnoreCase);
            missing.ExceptWith(userPerms);
            return missing;
        }
        ...
        public HashSet<string> FindNewStudents()
        {
            ...
            var result = new HashSet<string>(enrolledNow, StringComparer.OrdinalIgnoreCase);
        ...
        public HashSet<string> FindDroppedStudents()
        {
            ...
            var result = new HashSet<string>(enrolledLastQuarter, StringComparer.OrdinalIgnoreCase);
        ...
        public double CalculateRetentionRate()
        {
            ...
            var (_, last) = GetEnrollmentData();
            if (last == null || last.Count == 0) return 0.0;

            var continuing = FindContinuingStudents();
            double rate = (double)continuing.Count / (double)last.Count * 100.0;
            return rate;
        }
