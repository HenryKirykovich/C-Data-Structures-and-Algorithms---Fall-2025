    ...
    public class SpellChecker
    {
        ...
        public bool AnalyzeTextFile(string filename)
        {
            ...
            try
            {
                ...
                {
                    var norm = NormalizeWord(tok);
                    if (string.IsNullOrEmpty(norm)) continue;
                    allWordsInText.Add(norm);
                    uniqueWordsInText.Add(norm);
                }
        ...
        public void CategorizeWords()
        {
            ...
            if (uniqueWordsInText == null || uniqueWordsInText.Count == 0) return;

            foreach (var w in uniqueWordsInText)
            {
                if (dictionary.Contains(w))
                    correctlySpelledWords.Add(w);
                else
        ...
        public (bool inDictionary, bool inText, int occurrences) CheckWord(string word)
        {
            ...
            if (string.IsNullOrEmpty(norm)) return (false, false, 0);

            bool inDict = dictionary.Contains(norm);
            bool inTxt = uniqueWordsInText.Contains(norm);
            int occ = 0;
            if (allWordsInText != null && allWordsInText.Count > 0)
            {
                occ = allWordsInText.Count(w => string.Equals(w, norm, StringComparison.OrdinalIgnoreCase));
            }
        ...
        public List<string> GetMisspelledWords(int maxResults = 50)
        {
            ...
            if (misspelledWords == null || misspelledWords.Count == 0) return new List<string>();
            return misspelledWords.OrderBy(s => s).Take(maxResults).ToList();
        }
        ...
        public List<string> GetUniqueWordsSample(int maxResults = 20)
        {
            ...
            if (uniqueWordsInText == null || uniqueWordsInText.Count == 0) return new List<string>();
            return uniqueWordsInText.OrderBy(s => s).Take(maxResults).ToList();
        }
