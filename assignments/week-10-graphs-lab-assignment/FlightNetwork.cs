...
namespace Assignment10
{
    ...
    public class FlightNetwork
    {
        ...
        public void AddAirport(Airport airport)
        {
            ...
            {
                Console.WriteLine("[AddAirport] Invalid airport data provided.");
                return;
            }
            ...
            {
                // normalize stored code as uppercase
                airport.Code = code;
        ...
        public void AddFlight(Flight flight)
        {
            ...
            {
                Console.WriteLine("[AddFlight] Invalid flight data provided.");
                return;
            }
            ...
            // Ensure origin airport exists
            if (!airports.ContainsKey(originCode))
            {
                string city = airportCities.ContainsKey(originCode) ? airportCities[originCode] : originCode;
                var originAirport = new Airport(originCode, $"{originCode} Airport", city);
                AddAirport(originAirport);
            }

            // Ensure destination airport exists
            if (!airports.ContainsKey(destCode))
            {
                string city = airportCities.ContainsKey(destCode) ? airportCities[destCode] : destCode;
                var destAirport = new Airport(destCode, $"{destCode} Airport", city);
        ...
        public void LoadFlightsFromCSV(string filename)
        {
            ...
            for (int i = 1; i < lines.Length; i++)
            {
                ...
                try
                {
                    ...
                    if (parts.Length < 5)
                    {
                        Console.WriteLine($"[LoadFlightsFromCSV] Skipping malformed line {i + 1}: '{line}'");
                        continue;
                    }
                    ...
                    {
                        Console.WriteLine($"[LoadFlightsFromCSV] Invalid duration on line {i + 1}: '{parts[3]}'");
                        continue;
                    }

                    if (!decimal.TryParse(parts[4].Trim(), out decimal cost))
                    {
                        Console.WriteLine($"[LoadFlightsFromCSV] Invalid cost on line {i + 1}: '{parts[4]}'");
                        continue;
                    }

                    var flight = new Flight(origin, destination, airline, duration, cost);
                    AddFlight(flight);
                    flightsLoaded++;
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"[LoadFlightsFromCSV] Error parsing line {i + 1}: {ex.Message}");
                }
            }
        ...
        public void DisplayAllAirports()
        {
            ...
            Console.WriteLine($"Total airports in network: {airports.Count}");
            ...
            foreach (var airport in airports.Values.OrderBy(a => a.Code))
            {
                ...
                int connectionCount = routes.ContainsKey(code) ? routes[code].Count : 0;
                Console.WriteLine($"{airport.Code,-5} {airport.City,-20} {connectionCount,5}");
            }
        }
        ...
        public List<Flight> FindDirectFlights(string origin, string destination)
        {
            ...
            return routes[originUpper]
                .Where(f => string.Equals(f.Destination, destUpper, StringComparison.OrdinalIgnoreCase))
                .ToList();
        }
        ...
        public Flight? FindCheapestDirectFlight(string origin, string destination)
        {
            ...
            return options.OrderBy(f => f.Cost).First();
        }
        ...
        public List<string>? FindRoute(string origin, string destination)
        {
            ...
            while (queue.Count > 0)
            {
                ...
                foreach (var flight in routes[current])
                {
                    ...
                    if (neighbor.Equals(destUpper, StringComparison.OrdinalIgnoreCase))
                    {
                        var path = ReconstructPath(parents, originUpper, destUpper);
        ...
        public List<string>? FindCheapestRoute(string origin, string destination)
        {
            ...
            distances[originUpper] = 0m;
            queue.Enqueue(originUpper, 0m);

            while (queue.Count > 0)
            {
                queue.TryDequeue(out string? current, out decimal currentCost);
                if (current == null)
                {
                    break;
                }
                ...
                if (current.Equals(destUpper, StringComparison.OrdinalIgnoreCase))
                {
                    var path = ReconstructPath(parents, originUpper, destUpper);
                    return path.Count == 0 ? null : path;
                }
        ...
        {
            var validRoutes = new List<List<string>>();

            if (string.IsNullOrWhiteSpace(origin) || string.IsNullOrWhiteSpace(destination) || maxStops < 0 || maxCost < 0)
            {
                return validRoutes;
            }
            ...
            var currentPath = new List<string> { originUpper };
            var visited = new HashSet<string>(StringComparer.OrdinalIgnoreCase) { originUpper };

            DFSWithConstraints(originUpper, destUpper, maxStops, maxCost, 0m, currentPath, visited, validRoutes);
            ...
            decimal currentCost, List<string> currentPath, HashSet<string> visited, List<List<string>> validRoutes)
        {
            ...
            int stopsSoFar = currentPath.Count - 1;
        ...
        public string CalculateNetworkStatistics()
        {
            ...
            double avgConnections = routes.Count > 0 ? (double)totalFlights / routes.Count : 0.0;
            ...
            var allFlights = routes.Values.SelectMany(list => list).ToList();

            double avgCost = allFlights.Count > 0 ? (double)allFlights.Average(f => f.Cost) : 0.0;
            double avgDuration = allFlights.Count > 0 ? allFlights.Average(f => f.Duration) : 0.0;
            ...
            sb.AppendLine();
            sb.AppendLine($"Most Connected Airports ({maxConnections} connections): {string.Join(", ", mostConnected)}");
            sb.AppendLine($"Least Connected Airports ({minConnections} connections): {string.Join(", ", leastConnected)}");
            sb.AppendLine();
            sb.AppendLine($"Average Flight Cost: ${avgCost:F2}");
            sb.AppendLine($"Average Flight Duration: {avgDuration:F2} minutes ({avgDuration / 60.0:F2} hours)");
        ...
        public List<string> FindIsolatedAirports()
        {
            ...
            foreach (var flightList in routes.Values)
            {
                ...
                {
                    hasIncoming.Add(flight.Destination);
                }
            }
